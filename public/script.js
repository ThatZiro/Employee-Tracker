  ...
  try {
    ...
    switch (type) {
      ...
      case 'role':
        ...
        [results] = await database.execute('INSERT INTO role (job_title, department, salary) VALUES (?, ?, ?)', [
          object.role_name,
          department_id,
          object.role_salary,
        ]);
      ...
      case 'employee':
        ...
        [results] = await database.execute(
          ...
          [object.firstname, object.lastname, role_id, manager_id]
        );
        break;
      case 'employee_edit':
        let employee_id = object.employee.split(':')[0].trim();
        let employee_role_id = object.employee_role.split(':')[0].trim();
        [results] = await database.execute('UPDATE employees SET role_id = ? WHERE employee_id = ?', [
          employee_role_id,
          employee_id,
        ]);
        break;
